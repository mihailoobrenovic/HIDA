...
def _chunking_dot(big_matrix, small_matrix, chunk_size=100):
    # Make a copy if the array is not already contiguous
    small_matrix = np.ascontiguousarray(small_matrix)
    R = np.empty((big_matrix.shape[0], small_matrix.shape[1]), dtype=np.float32)
...
rgbcie_from_rgb = _chunking_dot(rgbcie_from_xyz, xyz_from_rgb)
rgb_from_rgbcie = _chunking_dot(rgb_from_xyz, xyz_from_rgbcie)
...
def _convert(matrix, arr):
    ...
    return _chunking_dot(arr, matrix.T.copy())
...
def xyz2luv(xyz, illuminant="D65", observer="2"):
    ...
    u0 = 4 * xyz_ref_white[0] / _chunking_dot([1, 15, 3], xyz_ref_white)
    v0 = 9 * xyz_ref_white[1] / _chunking_dot([1, 15, 3], xyz_ref_white)
...
def luv2xyz(luv, illuminant="D65", observer="2"):
    ...
    # reference white x,z
    uv_weights = [1, 15, 3]
    u0 = 4 * xyz_ref_white[0] / _chunking_dot(uv_weights, xyz_ref_white)
    v0 = 9 * xyz_ref_white[1] / _chunking_dot(uv_weights, xyz_ref_white)
...
def separate_stains(rgb, conv_matrix):
    ...
    * ``hpx_from_rgb``: Hematoxylin + PAS
    ...
    """
    rgbshape = rgb.shape
    #rgb=dtype.img_as_float(rgb)
    rgb = dtype.convert(rgb, np.float32)
    rgb += 2
    rgb = _chunking_dot(np.reshape(-np.log(rgb), (-1, 3)), conv_matrix)
    return np.reshape(rgb, rgbshape)
...
def combine_stains(stains, conv_matrix):
    ...
    * ``rgb_from_hpx``: Hematoxylin + PAS
    ...
    stainsshape = stains.shape
    #stains = dtype.img_as_float(stains)
    stains = dtype.convert(stains, np.float32)
    stains = _chunking_dot(-np.reshape(stains, (-1, 3)), conv_matrix)
    stains = np.exp(stains)
    return rescale_intensity(np.reshape(stains - 2, stainsshape),
                             in_range=(-1, 1))
